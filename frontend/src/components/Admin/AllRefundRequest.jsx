    ...
    try {
      for (const stockItem of newStock) {
        ...
        if ( stockItem.size==size ) {
          ...
          const response = await axios.patch(`${server}/product/update-stock/${productId}`, {
    ...
    },
    {
      ...
      renderCell: (params) => (
        ...
        onClick={async() =>{
         ...
         const response=await axios.patch(`${server}/kuchvi/update-kuchvi/${params.row.kuchviId}`, {
